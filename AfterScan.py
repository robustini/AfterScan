...
02/10/2022: 1.12: JRE
    - Fix bug: Do not enable Video checkbox is Crop checkbox is unselected (need uniform frames to generate video)
03/10/2022: 1.12: JRE
...
def display_video_encoding_warning():
    ...
    video_encoding_label = Label(video_encoding_warning, text='\rThis utility uses FFmpeg to generate video from S8/R8 frames produced by a film scanner.\r\n'
    ...
    if ConvertLoopRunning:
        ...
        if GenerateVideo:
            ...
            if TargetVideoFilename == "":   # if filename not defined assign a default one
                TargetVideoFilename = "8mm-AfterScan-" + datetime.now().strftime("%Y_%m_%d-%H-%M-%S") + ".mp4"
